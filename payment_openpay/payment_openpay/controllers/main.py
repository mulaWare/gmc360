...
class OpenpayController(http.Controller):
    ...
    @http.route(['/payment/openpay/s2s/create_json'], type='json', auth='public')
    ...
    @http.route(['/payment/openpay/s2s/create_json_3ds'], type='json', auth='public', csrf=False)
    ...
    @http.route(['/payment/openpay/create_charge'], type='json', auth='public')
    def openpay_create_charge(self, **post):
        ...
        if tx.type == 'form_save' and tx.partner_id:
            ...
            response = tx._create_openpay_charge(acquirer_ref=payment_token_id.acquirer_ref, email=openpay_token['email'])
        else:
            response = tx._create_openpay_charge(tokenid=openpay_token['id'], email=openpay_token['email'])
        _logger.info('Openpay: entering form_feedback with post data %s', pprint.pformat(response))
        if response:
            request.env['payment.transaction'].sudo().with_context(lang=None).form_feedback(response, 'openpay')
