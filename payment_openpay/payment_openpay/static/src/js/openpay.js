odoo.define('payment_openpay.openpay', function(require) {
    ...
    // https://openpay.com/docs/currencies#zero-decimal
    var int_currencies = [
    ...
    var openpayHandler;
    function getOpenpayHandler()
    {
        if (openpayHandler) {
            return openpayHandler;
        }
        var handler = openpayHandler = OpenpayCheckout.configure({
            key: $("input[name='openpay_key']").val(),
            image: $("input[name='openpay_image']").val(),
            locale: 'auto',
            token: function(token, args) {
                ...
                ajax.jsonRpc("/payment/openpay/create_charge", 'call', {
                    ...
                    acquirer_id: $("#acquirer_openpay").val(),
                    ...
                    var wizard = $(qweb.render('openpay.error', {'msg': msg || _t('Payment error')}));
        ...
        for(var i=0; i<mutations.length; ++i) {
            for(var j=0; j<mutations[i].addedNodes.length; ++j) {
                if(mutations[i].addedNodes[j].tagName.toLowerCase() === "form" && mutations[i].addedNodes[j].getAttribute('provider') == 'openpay') {
                    display_openpay_form($(mutations[i].addedNodes[j]));
                }
            }
        }
    });
    ...
    function display_openpay_form(provider_form) {
        ...
        var acquirer_id = parseInt(provider_form.find('#acquirer_openpay').val());
        ...
        ajax.jsonRpc(payment_tx_url, 'call', {
            ...
            var $pay_openpay = $('#pay_openpay').detach();
            try { provider_form[0].innerHTML = data; } catch (e) {}
            // Restore 'Pay Now' button HTML since data might have changed it.
            $(provider_form[0]).find('#pay_openpay').replaceWith($pay_openpay);
        }).done(function () {
            getOpenpayHandler().open({
                name: merchant,
    ...
    $.getScript("https://checkout.openpay.com/checkout.js", function(data, textStatus, jqxhr) {
        observer.observe(document.body, {childList: true});
        display_openpay_form($('form[provider="openpay"]'));
    });
});
